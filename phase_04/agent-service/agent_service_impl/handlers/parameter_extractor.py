...
def extract_complete_task_params(message: str) -> ParameterExtractionResult:
    ...
    # Try to extract numeric task ID - multiple patterns
    id_patterns = [
        r"(?:task\s+)?#?(\d+)",  # task 5, #5, 5
        r"(?:complete|finish|done|mark)\s+(?:task\s+)?(\d+)",  # complete 5
        r"(\d+)\s+(?:is\s+)?(?:done|complete|finished)",  # 5 done
    ]
...
def extract_update_task_params(message: str) -> ParameterExtractionResult:
    ...
    # Extract task ID - multiple patterns
    id_patterns = [
        r"(?:update|change|edit|modify|rename)\s+(?:task\s+)?#?(\d+)",  # update task 3
        r"(?:task\s+)?#?(\d+)\s+(?:to|:)",  # task 3 to
    ]
    ...
    # Extract new title - look for text after "to" or ":"
    title_patterns = [
        r"\bto\s+['\"]?(.+?)['\"]?\s*$",  # "to buy milk"
        r":\s*['\"]?(.+?)['\"]?\s*$",  # ": buy milk"
        r"(?:update|change|edit|rename)\s+(?:task\s+)?\d+\s+(?:to\s+)?['\"]?(.+?)['\"]?\s*$",  # full pattern
    ]
    ...
    for pattern in title_patterns:
        ...
        if match:
            ...
            # Make sure we're not capturing just "to" or empty string
            if new_value and len(new_value) > 2 and new_value.lower() not in ["to", "the"]:
