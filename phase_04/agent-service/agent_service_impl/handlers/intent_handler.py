...
def classify_intent(message: str) -> IntentResult:
    ...
    complete_patterns = [
        ...
        # Additional patterns
        (r"^mark\s+task\s+\d+\s+(as\s+)?(done|complete)", 0.98),  # "mark task 1 as done"
        (r"^(complete|finish)\s+task\s+\d+", 0.98),  # "complete task 1"
        (r"^(i\s+)?have\s+(done|completed|finished)\s+\d+", 0.90),  # "I have done 1"
        (r"^tick\s+(off\s+)?(task\s+)?\d+", 0.90),  # "tick off 1"
    ]
    ...
    update_patterns = [
        ...
        # Additional patterns
        (r"^(update|change|edit|modify)\s+task\s+\d+", 0.98),  # "update task 1"
        (r"^(set|change)\s+task\s+\d+\s+to", 0.95),  # "set task 1 to"
        (r"^edit\s+\d+\s+to\s+", 0.95),  # "edit 1 to"
    ]
